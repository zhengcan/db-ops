...
- 每次执行创建一个带时间戳的目录。
  - 未指定 `--dir` 时（默认行为）：`backup/mysql/<label>/<YYYYMMDD_HHMMSS>/`
    （相对当前工作目录）。`<label>` 的取值：若本次运行通过 `dbs.conf`/
    `--instance` 解析出了一个配置实例，则使用该实例名（比稳定、可读，不受
    主机名/IP 变化影响）；否则使用 `--host`/`$DB_HOST` 经清洗后的值。两种
    情况下都会经过同一个清洗步骤，仅保留字母、数字、`.`、`-`、`_`，其余字符
    （尤其是 `/`）替换为 `_`，防止异常输入破坏目录结构或产生路径穿越风险。
